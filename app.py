...
# 4. SISTEMA DE BLOQUEIO INDEPENDENTE (Ajustado para nunca bloquear a palavra Outros)
if not dados_existentes.empty and "Comida" in dados_existentes.columns:
    # Pegamos o que está na planilha, mas ignoramos a palavra "Outros" na hora de bloquear
    comidas_ocupadas = [
...
# Garantimos que "Outros" sempre seja adicionado no final da lista de escolhas
...
if enviado:
    if nome and whatsapp and comida_final and comida_final.strip():
        # Validação extra: impede a pessoa de escrever apenas "Outros" no campo de texto livre
        if comida_final.strip().lower() == "outros":
            st.error("Por favor, especifique o nome real do prato que você vai trazer (não escreva apenas 'Outros').")
        else:
